 ...
 * the parallel and net-decomposing netlist router (DecompNetlistRouter). Works
 * like \see ParallelNetlistRouter, but tries to "decompose" nets and assign
 * them to the next level of the partition tree where possible, driving
 * per-thread NewrtConnectionRouters.
...
const int NEWRT_MAX_DECOMP_ITER = 5;
...
const int NEWRT_MAX_DECOMP_DEPTH = 2;

/** Minimum # of fanouts of a net to consider decomp. */
const int NEWRT_MIN_DECOMP_SINKS = 8;

/** Minimum # of fanouts of a virtual net to consider decomp. */
const int NEWRT_MIN_DECOMP_SINKS_VNET = 8;
...
class NewrtNetlistRouter : public NetlistRouter {
  ...
  private:
    ...
    NewrtConnectionRouter<HeapType> _make_router(const RouterLookahead* router_lookahead, bool is_flat) {
        auto& device_ctx = g_vpr_ctx.device();
        auto& route_ctx = g_vpr_ctx.mutable_routing();

        return NewrtConnectionRouter<HeapType>(
            device_ctx.grid,
            *router_lookahead,
    ...
    /** Per-thread storage for NewrtConnectionRouter. */
    tbb::enumerable_thread_specific<NewrtConnectionRouter<HeapType>> _routers_th;
