...
class newrt_barrier_mutex_t {
  ...
  public:
     ...
     */
    explicit inline newrt_barrier_mutex_t(size_t num_threads)
        : count_(num_threads)
...
class newrt_barrier_spin_t {
  ...
  public:
     ...
     * any thread is allowed to proceed
     */
    explicit inline newrt_barrier_spin_t(size_t num_threads) { num_threads_ = num_threads; }
 ...
 */
class newrt_standard_barrier_t {
  ...
  public:
     ...
     *      to proceed.
     */
    explicit inline newrt_standard_barrier_t(size_t num_threads)
...
using newrt_barrier_t = newrt_standard_barrier_t; // Using the standard thread barrier

/**
 * @class NewrtConnectionRouter implements the MultiQueue-based parallel connection
...
class NewrtConnectionRouter : public ConnectionRouter<MultiQueueDAryHeap<HeapImplementation::arg_D>> {
  public:
    NewrtConnectionRouter(
        ...
        for (int i = 0; i < multi_queue_num_threads - 1; ++i) {
            this->sub_threads_[i] = std::thread(&NewrtConnectionRouter::timing_driven_find_single_shortest_path_from_heap_sub_thread_wrapper, this, i + 1 /*0: main thread*/);
        }
    }
    ...
    ~NewrtConnectionRouter() {
        ...
        VTR_LOG("NEWRT Connection Router is being destroyed. Time spent on path search: %.3f seconds.\n",
 ...
 * errors since heap type specializations won't get emitted from newrt_connection_router.cpp
 * without it.
 * The alternative is moving all NewrtConnectionRouter fn implementations into the header. */
std::unique_ptr<ConnectionRouterInterface> make_newrt_connection_router(
