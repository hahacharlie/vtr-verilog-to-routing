...
/** @file Impls for NewrtNetlistRouter */
...
inline RouteIterResults NewrtNetlistRouter<HeapType>::route_netlist(int itry, float pres_fac, float worst_neg_slack) {
...
template<typename HeapType>
void NewrtNetlistRouter<HeapType>::handle_bb_updated_nets(const std::vector<ParentNetId>& nets) {
    VTR_ASSERT(_tree);
    _tree->update_nets(nets);
}

template<typename HeapType>
void NewrtNetlistRouter<HeapType>::set_rcv_enabled(bool x) {
    if (x)
        VPR_FATAL_ERROR(VPR_ERROR_ROUTE, "Net decomposition with RCV is not implemented yet.\n");
}

template<typename HeapType>
void NewrtNetlistRouter<HeapType>::set_timing_info(std::shared_ptr<SetupHoldTimingInfo> timing_info) {
    _timing_info = timing_info;
}

/** Get a sink mask for sinks inside a vnet's clipped BB. */
inline vtr::dynamic_bitset<> newrt_get_vnet_sink_mask(const VirtualNet& vnet) {
...
template<typename HeapType>
bool NewrtNetlistRouter<HeapType>::should_decompose_net(ParentNetId net_id, const PartitionTreeNode& node) {
    ...
    if (_itry > NEWRT_MAX_DECOMP_ITER)
        return false;
    /* Net is too small */
    int num_sinks = _net_list.net_sinks(net_id).size();
    if (num_sinks < NEWRT_MIN_DECOMP_SINKS)
        return false;
...
inline bool newrt_should_decompose_vnet(const VirtualNet& vnet, const PartitionTreeNode& node) {
    /* We're at a partition tree leaf: no more nodes to delegate newly created vnets to */
    if (!node.left || !node.right)
        return false;

    /* Vnet has been decomposed too many times */
    if (vnet.times_decomposed >= NEWRT_MAX_DECOMP_DEPTH)
        return false;
    ...
    /* Vnet is too small */
    int num_sinks = newrt_get_vnet_sink_mask(vnet).count();
    if (num_sinks < NEWRT_MIN_DECOMP_SINKS_VNET)
        return false;
...
template<typename HeapType>
void NewrtNetlistRouter<HeapType>::route_partition_tree_node(tbb::task_group& g, PartitionTreeNode& node) {
    ...
    for (size_t i : order) {
        if (i < nets.size()) { /* Regular net (not decomposed) */
            ...
            if (!flags.success && !flags.retry_with_full_bb) {
                /* Disconnected RRG and NewrtConnectionRouter doesn't think growing the BB will work */
                _results_th.local().is_routable = false;
                return;
            }
            if (flags.retry_with_full_bb) {
                /*NewrtConnectionRouter thinks we should grow the BB. Do that and leave this net unrouted for now */
            ...
            if (newrt_should_decompose_vnet(vnet, node)) {
                VirtualNet left_vnet, right_vnet;
            ...
            auto sink_mask = newrt_get_vnet_sink_mask(vnet);
            route_net(
                _routers_th.local(),
                _net_list,
                vnet.net_id,
...
inline t_bb newrt_clip_to_side(const t_bb& bb, Axis axis, int cutline_pos, Side side) {
    t_bb out = bb;
...
inline void newrt_make_vnet_pair(ParentNetId net_id, const t_bb& bb, Axis cutline_axis, int cutline_pos, VirtualNet& left, VirtualNet& right) {
    left.net_id = net_id;
    left.clipped_bb = newrt_clip_to_side(bb, cutline_axis, cutline_pos, Side::LEFT);
    right.net_id = net_id;
    right.clipped_bb = newrt_clip_to_side(bb, cutline_axis, cutline_pos, Side::RIGHT);
}

template<typename HeapType>
bool NewrtNetlistRouter<HeapType>::decompose_and_route_net(ParentNetId net_id, const PartitionTreeNode& node, VirtualNet& left, VirtualNet& right) {
    auto& route_ctx = g_vpr_ctx.mutable_routing();
    ...
    newrt_make_vnet_pair(net_id, net_bb, node.cutline_axis, node.cutline_pos, left, right);
...
inline std::string newrt_describe_bbox(const t_bb& bb) {
    return std::to_string(bb.xmin) + "," + std::to_string(bb.ymin)
           + "x" + std::to_string(bb.xmax) + "," + std::to_string(bb.ymax);
}

inline std::string newrt_describe_rr_coords(RRNodeId inode) {
...
inline std::string newrt_describe_vnet(const VirtualNet& vnet) {
    const auto& route_ctx = g_vpr_ctx.routing();

    std::string out = "";
    out += "Virtual net with bbox " + newrt_describe_bbox(vnet.clipped_bb)
           + " parent net: " + std::to_string(size_t(vnet.net_id))
           + " parent bbox: " + newrt_describe_bbox(route_ctx.route_bb[vnet.net_id]) + "\n";

    RRNodeId source_rr = route_ctx.net_rr_terminals[vnet.net_id][0];
    out += "source: " + newrt_describe_rr_coords(source_rr) + ", sinks:";
    for (size_t i = 1; i < route_ctx.net_rr_terminals[vnet.net_id].size(); i++) {
        RRNodeId sink_rr = route_ctx.net_rr_terminals[vnet.net_id][i];
        out += " " + newrt_describe_rr_coords(sink_rr);
    }
    out += "\n";

    const auto& vnet_isinks = newrt_get_vnet_sink_mask(vnet);
    ...
    for (auto it = all_nodes.begin(); it != all_nodes.end(); ++it) {
        if ((*it).is_leaf()) {
            out += newrt_describe_rr_coords((*it).inode) + " END ";
            ++it;
            if (it == all_nodes.end())
                break;
            out += newrt_describe_rr_coords((*it).parent()->inode) + " -> ";
            out += newrt_describe_rr_coords((*it).inode) + " -> ";
        } else {
            out += newrt_describe_rr_coords((*it).inode) + " -> ";
        }
    }
...
template<typename HeapType>
bool NewrtNetlistRouter<HeapType>::decompose_and_route_vnet(VirtualNet& vnet, const PartitionTreeNode& node, VirtualNet& left, VirtualNet& right) {
    /* Sample enough sinks to provide branch-off points to the virtual nets we create */
    ...
    if (!flags.success) { /* Even if flags.retry_with_full_bb is set, better to bail out here */
        PartitionTreeDebug::log("Failed to route decomposed net:\n" + newrt_describe_vnet(vnet));
        return false;
    }

    /* Divide the net into two halves */
    newrt_make_vnet_pair(vnet.net_id, vnet.clipped_bb, node.cutline_axis, node.cutline_pos, left, right);
...
inline bool newrt_is_close_to_cutline(RRNodeId inode, Axis cutline_axis, int cutline_pos, int thickness) {
...
inline bool newrt_is_close_to_bb(RRNodeId inode, const t_bb& bb, int thickness) {
    const auto& device_ctx = g_vpr_ctx.device();
    const auto& rr_graph = device_ctx.rr_graph;
...
inline bool newrt_get_reduction_mask(ParentNetId net_id, Axis cutline_axis, int cutline_pos, vtr::dynamic_bitset<>& out) {
    const auto& route_ctx = g_vpr_ctx.routing();
    ...
    t_bb sink_side_bb = newrt_clip_to_side(net_bb, cutline_axis, cutline_pos, !source_side);
    ...
    for (int isink = 1; isink < num_sinks + 1; isink++) {
        ...
        if (inside_bb(rr_sink, sink_side_bb)) {
            ...
            if (newrt_is_close_to_cutline(rr_sink, cutline_axis, cutline_pos, 1)) /* Don't count sinks close to cutline */
                continue;
            all_sinks++;
        }
    }
...
template<typename HeapType>
vtr::dynamic_bitset<> NewrtNetlistRouter<HeapType>::get_decomposition_mask(ParentNetId net_id, const PartitionTreeNode& node) {
    const auto& route_ctx = g_vpr_ctx.routing();
     ...
     * is close to cutline. */
    bool is_reduced = newrt_get_reduction_mask(net_id, node.cutline_axis, node.cutline_pos, out);

    bool source_on_cutline = newrt_is_close_to_cutline(tree.root().inode, node.cutline_axis, node.cutline_pos, 1);
    ...
    for (size_t isink = 1; isink < num_sinks + 1; isink++) {
        ...
        RRNodeId rr_sink = route_ctx.net_rr_terminals[net_id][isink];
        if (newrt_is_close_to_cutline(rr_sink, node.cutline_axis, node.cutline_pos, 1))
            out.set(isink, true);
    }
...
inline int newrt_get_reduction_mask_vnet_no_source(const VirtualNet& vnet, Axis cutline_axis, int cutline_pos, vtr::dynamic_bitset<>& out) {
    const auto& route_ctx = g_vpr_ctx.routing();
    ...
    t_bb left_side = newrt_clip_to_side(net_bb, cutline_axis, cutline_pos, Side::LEFT);
    t_bb right_side = newrt_clip_to_side(net_bb, cutline_axis, cutline_pos, Side::RIGHT);
    auto& is_isink_reached = tree.get_is_isink_reached();
    ...
    for (const t_bb& side_bb : {left_side, right_side}) {
        ...
        for (int isink = 1; isink < num_sinks + 1; isink++) {
            ...
            if (is_narrow) /* If the box is narrow, don't check for all_sinks -- we are going to reduce it anyway */
                continue;
            if (newrt_is_close_to_bb(rr_sink, side_bb, 1))
                continue;
            all_sinks++;
            if (all_sinks > MIN_SINKS) {
                should_reduce = false;
...
/** Similar fn to \see newrt_get_reduction_mask, but works with virtual nets
 * and checks against the clipped bounding box instead of the cutline when counting sink-side sinks. */
inline bool newrt_get_reduction_mask_vnet_with_source(const VirtualNet& vnet, Axis cutline_axis, int cutline_pos, vtr::dynamic_bitset<>& out) {
    const auto& route_ctx = g_vpr_ctx.routing();
    ...
    for (int isink = 1; isink < num_sinks + 1; isink++) {
        ...
        if (inside_bb(rr_sink, sink_side_bb)) {
            if (!is_isink_reached.get(isink))
                sink_side_mask.set(isink, true);
            if (newrt_is_close_to_bb(rr_sink, sink_side_bb, 1)) /* Don't count sinks close to BB */
                continue;
            all_sinks++;
        }
    }
...
template<typename HeapType>
vtr::dynamic_bitset<> NewrtNetlistRouter<HeapType>::get_decomposition_mask_vnet(const VirtualNet& vnet, const PartitionTreeNode& node) {
    const auto& route_ctx = g_vpr_ctx.routing();
    const RouteTree& tree = route_ctx.route_trees[vnet.net_id].value();
    int num_sinks = tree.num_sinks();
    ...
    if (inside_bb(tree.root().inode, vnet.clipped_bb)) { /* We have source, no need to sample after reduction in most cases */
        bool is_reduced = newrt_get_reduction_mask_vnet_with_source(vnet, node.cutline_axis, node.cutline_pos, out);
        bool source_on_cutline = newrt_is_close_to_cutline(tree.root().inode, node.cutline_axis, node.cutline_pos, 1);
        if (!is_reduced || source_on_cutline) {
            convex_hull_downsample(vnet.net_id, vnet.clipped_bb, out);
        }
    } else {
        int reduced_sides = newrt_get_reduction_mask_vnet_no_source(vnet, node.cutline_axis, node.cutline_pos, out);
        if (reduced_sides < 2) {
            convex_hull_downsample(vnet.net_id, vnet.clipped_bb, out);
        }
    }

    std::vector<size_t> isinks = sink_mask_to_vector(newrt_get_vnet_sink_mask(vnet), tree.num_sinks());
    ...
    for (size_t isink : isinks) {
        ...
        if (newrt_is_close_to_bb(rr_sink, vnet.clipped_bb, 1))
            out.set(isink, true);
    }
